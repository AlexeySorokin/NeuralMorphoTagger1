...
from common.read import read_tags_infile, extract_frequent_words
...
if __name__ == '__main__':
    ...
    if params["train_files"] is not None:
        ...
        train_data = []
        token_reading_keys = ["word_column", "pos_column", "tag_column"]
        if symbols_has_tokens:
            frequent_pairs = extract_frequent_words(
                params["train_files"], **token_reading_params)
            token_reading_keys += ["to_lower", "append_case"]
        else:
            frequent_pairs = None
        token_reading_params = {key: token_reading_params[key]
                                for key in token_reading_keys if key in token_reading_params}
