...
class NeuralLM:
    ...
    def to_json(self, outfile, model_file):
        ...
        for (attr, val) in inspect.getmembers(self):
            if not (attr.startswith("__") or inspect.ismethod(val) or
                    ...
                    attr in ["callbacks", "model_", "_attention_func_", "hidden_state_func_"]):
                info[attr] = val
    ...
    def _build_symbol_layer(self, symbol_inputs):
        ...
        if self.use_embeddings:
            if self.symbols_has_features:
                dense_layer = kl.Dense(self.embeddings_size, use_bias=False,
                                       name="symbol_embeddings")
                symbol_embeddings = kl.TimeDistributed(
                    dense_layer, name="distributed_embeddings")(symbol_inputs)
            else:
                symbol_embeddings = kl.Embedding(self.input_symbols_number,
